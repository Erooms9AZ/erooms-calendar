...
// --- Load external config from Apps Script ---
async function loadConfig() {
  const url = "https://script.google.com/macros/s/AKfycbxzW6PrNFeoYLGKx4ugcUSpNa9n_QTCi7GAPknr4Bw0XOYrsebqhJ2uGbx4FSNV2-70Wg/exec";

  const res = await fetch(url);
  const text = await res.text();

  // Apps Script returns JS as plain text, so we execute it
  eval(text);

  if (!window.EROOMS_CONFIG || !window.EROOMS_CONFIG.apiKey) {
    throw new Error("Config did not load");
  }

  return window.EROOMS_CONFIG;
}

// Placeholder for API key (assigned after config loads)
let apiKey;
...
(async function () {
  ...
  // Load config first, then assign apiKey
